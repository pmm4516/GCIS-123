...
def draw_square(length):
    perimeter = 0
    t.fillcolor(get_color(gen_number()))
    t.begin_fill()
...
def draw_line(total_squares, length):
    ...
    while x < total_squares:
        total_perimeter += draw_square(length)
        t.forward(length)
        x += 1
...
def main():
    total_squares = int(input("Enter the number of squares in the line: "))
    length = int(input("Enter the length of the square: "))
    draw_line(total_squares, length)
